...
eureka_endpoint=$(echo 'y' | gcloud compute instances describe --zone us-east1-b $vm_id 2>&1 | grep -A 1 eureka_endpoint | grep value | awk '{print $2}')
response=$(curl -s $eureka_endpoint/api/_internal/scripts?vm_id=$vm_id)

script=$(jq '.script' <<<$response | sed -r 's/^"(.*)"$/\1/' | sed 's/\\"/"/g')
